...
/**
 * Mobile Bottom Navigation with FAB - Large Size Preset
 *
 * LARGE PRESET - Accessibility-first with larger touch targets
 *
 * Spacing Configuration:
 * - Nav bar height: 64px (h-16)
 * - FAB button: 64px × 64px (w-16 h-16)
 * - Bottom offset: 24px (bottom-6)
 * - FAB right gap: 24px (right-6)
 * - Nav right gap: 112px (right-28) = 64px + (24px × 2)
 *
 * SPACING FORMULA: navRightGap = fabSize + (fabGap × 2)
 * Verification: 4rem + (1.5rem × 2) = 7rem (28 × 0.25rem = 112px) ✓
 */

const SPACING = {
  navHeight: 'h-16',                    // 64px (4rem)
  navBottomOffset: 'bottom-6',          // 24px (1.5rem)
  navLeftOffset: 'left-4',              // 16px (1rem)
  navRightGap: 'right-28',              // 112px (7rem)

  fabSize: 'w-16 h-16',                 // 64px × 64px (4rem)
  fabBottomOffset: 'bottom-6',          // 24px (1.5rem)
  fabRightGap: 'right-6',               // 24px (1.5rem)

  navIconSize: 24,
  fabIconSize: 20,
  fabMenuIconSize: 20,

  contentBottomPadding: 'pb-[calc(6rem+env(safe-area-inset-bottom))]',
};
...
const MobileBottomNav = () => {
  ...
  return (
    <>
      ...
      >
        <div className={`flex items-center justify-around ${SPACING.navHeight} px-3`}>
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                {active ? (
                  <div className={`flex flex-row items-center justify-center gap-2 px-4 py-2.5 rounded-2xl ${THEME.activeBg} backdrop-blur-md shadow-lg border ${THEME.border} transition-all duration-300`}>
      ...
      {showMenu && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-40 animate-in fade-in duration-200" onClick={() => setShowMenu(false)}>
          <div
            className={`absolute bottom-[5.5rem] ${SPACING.fabRightGap} ${THEME.menuBg} backdrop-blur-lg rounded-2xl shadow-2xl border ${THEME.border} overflow-hidden animate-in slide-in-from-bottom-4 duration-300 transform translate-z-0`}
              ...
              {fabMenuItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className="flex items-center gap-4 px-5 py-4 hover:bg-white/10 rounded-xl transition-all duration-200 active:scale-95"
                  >
